...
El rendimiento del procedimiento de normalización de la TMM puede examinarse mediante gráficos de diferencia media o __plot MD__. Esto visualiza el cambio de pliegue ajustado al tamaño de la biblioteca entre dos librerías (la diferencia) contra el promedio de la expresión de log a través de esas librerías (la media). El plot MD de pliegue se genera comparando la muestra 1 ( _GTEX.111CU.0226.SM.5GZXC_ ) con una librería artificial construida a partir de la media de todas las demás muestras.
...
```{r fig.cap="Plot MD para la normalización de los conteos", echo=FALSE, out.extra='', fig.height=5, fig.width=5, fig.align="center"}
plotMD(cpm(y, log=TRUE), column=1)
abline(h=0, col="red", lty=2, lwd=2)
```
```{r saveplotMD, echo=FALSE, results='hide'}
#Guardo el plotMD en un archivo tiff en el directorio figures
tiff("./figures/plotMD.tiff", res = 200, width = 4, height = 4, units = 'in')
plotMD(cpm(y, log=TRUE), column=1)
abline(h=0, col="red", lty=2, lwd=2)
dev.off()
```
